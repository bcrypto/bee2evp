...
типом NULL. При кодировании (декодировании) синхропосылка читается
(записывается) из (в) поле oiv контекста ctx.
...
#define FLAGS_belt_ecb                                                         \
	(EVP_CIPH_ECB_MODE | EVP_CIPH_CTRL_INIT | EVP_CIPH_RAND_KEY |              \
...
#define FLAGS_belt_cbc                                                         \
	(EVP_CIPH_CBC_MODE | EVP_CIPH_CTRL_INIT | EVP_CIPH_RAND_KEY |              \
		EVP_CIPH_CUSTOM_COPY | EVP_CIPH_FLAG_DEFAULT_ASN1)
...
#define FLAGS_belt_cfb                                                         \
	(EVP_CIPH_CFB_MODE | EVP_CIPH_CTRL_INIT | EVP_CIPH_RAND_KEY |              \
		EVP_CIPH_CUSTOM_COPY | EVP_CIPH_FLAG_DEFAULT_ASN1)
...
#define FLAGS_belt_ctr                                                         \
	(EVP_CIPH_CTR_MODE | EVP_CIPH_CTRL_INIT | EVP_CIPH_ALWAYS_CALL_INIT |      \
		EVP_CIPH_RAND_KEY | EVP_CIPH_CUSTOM_COPY | EVP_CIPH_CUSTOM_IV |        \
	...
	EVP_CIPHER_CTX* ctx, octet* out, const octet* in, size_t inlen)
{
	...
	if (!in)
	{
		...
		if (state->block_len != 8 || !beltDWPStepV(state->block, state->state))
			return -1;
		return 0;
	}
	// открытые данные?
...
#define FLAGS_belt_kwp                                                         \
	(EVP_CIPH_WRAP_MODE | EVP_CIPH_CTRL_INIT | EVP_CIPH_RAND_KEY |             \
		EVP_CIPH_FLAG_CUSTOM_CIPHER | EVP_CIPH_CUSTOM_COPY |                   \
		EVP_CIPH_CUSTOM_IV | EVP_CIPH_FLAG_DEFAULT_ASN1)
...
static int evpBeltKWP_set_asn1_params(EVP_CIPHER_CTX* ctx, ASN1_TYPE* params)
{
	params->type = V_ASN1_NULL;
	return 1;
}

static int evpBeltKWP_get_asn1_params(EVP_CIPHER_CTX* ctx, ASN1_TYPE* params)
{
	return params->type == V_ASN1_NULL;
}
...
int evpBeltCipher_bind(ENGINE* e)
{
	...
	return ENGINE_register_ciphers(e) && EVP_add_cipher(EVP_belt_ecb128) &&
		EVP_add_cipher(EVP_belt_ecb192) && EVP_add_cipher(EVP_belt_ecb256) &&
		EVP_add_cipher(EVP_belt_cbc128) && EVP_add_cipher(EVP_belt_cbc192) &&
		EVP_add_cipher(EVP_belt_cbc256) && EVP_add_cipher(EVP_belt_cfb128) &&
		EVP_add_cipher(EVP_belt_cfb192) && EVP_add_cipher(EVP_belt_cfb256) &&
