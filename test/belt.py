...
class Belt_test:
	...
	'''def beltKWPEncr(self, right_plain_msg, right_enc_msg, key, msg_len):
		plain_msg = (right_plain_msg[:(msg_len/4)]+'00').decode('hex')
		with open("plain_msg.txt", "wb") as f:
			f.write(plain_msg)
		print(right_plain_msg, msg_len)
		key_bitlen = len(key)*4
		cmd = 'enc -belt-kwp{} -{} -e -in {} -K {}'.format(key_bitlen, self.pad, 'plain_msg.txt', key)
		#retcode, out, er__ = openssl('enc -belt-ecb256 -nopad -e -in plain_msg.txt -K e9dee72c8f0c0fa62ddb49f46f73964706075316ed247a3739cba38303a98bf6', True)
		retcode, out, er__ = openssl(cmd, True)
		print(out.encode("hex"))
		print(right_enc_msg[:(msg_len/4)])
		assert right_enc_msg[:(msg_len/4)] == out.encode('hex')[:(msg_len/4)]

	def beltKWPDecr(self, right_plain_msg, right_enc_msg, key, msg_len):
		if (msg_len % 64 !=0):
			self.pad = '-nopad'
		enc_msg = right_enc_msg[:(msg_len/4)].decode('hex')
		with open("enc_msg.txt", "wb") as f:
			f.write(enc_msg)
		key_bitlen = len(key)*4
		cmd = 'enc -belt-kwp{} -{} -e -in {} -K {}'.format(key_bitlen, self.pad, 'enc_msg.txt', key)
		#retcode, out, er__ = openssl('enc -belt-ecb256 -nopad -e -in plain_msg.txt -K e9dee72c8f0c0fa62ddb49f46f73964706075316ed247a3739cba38303a98bf6', True)
		retcode, out, er__ = openssl(cmd, True)
		assert right_plain_msg[:(msg_len/4)] == out.encode('hex')[:(msg_len/4)]'''

	'''@staticmethod	
	def test_beltECBEncr(plain_msg, key):
		with open("plain_msg.txt", "w") as f:
			f.read(plain_msg)
		cmd = 'enc -belt-ecb256 -nopad -e '
		retcode, out, er__ = openssl('enc -belt-ecb256 -nopad -e -in plain_msg.txt -K e9dee72c8f0c0fa62ddb49f46f73964706075316ed247a3739cba38303a98bf6', True)
	...
	@staticmethod
	def test_beltECBDecr(encrypted_msg):
		with open("plain_msg.txt", "w") as f:
			f.read(plain_msg) 
		retcode, out, er__ = openssl('enc -belt-ecb256 -nopad -d -in plain_msg.txt -K e9dee72c8f0c0fa62ddb49f46f73964706075316ed247a3739cba38303a98bf6', True)
		encrypted_msg = out.encode("hex")
		assert encrypted_msg == '69cca1c93557c9e3d66bc3e0fa88fa6e5f23102ef109710775017f73806da9dc46fb2ed2ce771f26dcb5e5d1569f9ab0'
		print("encrypted message:\n" + encrypted_msg)'''
...
if __name__ == "__main__":
	...
	'''belt.beltCBCEncr(right_plain_msg, right_enc_msg, key, 384)
	belt.beltCFBEncr(right_plain_msg, right_enc_msg, key, 384)
	belt.beltCTRDecr(right_plain_msg, right_enc_msg, key, 384)
	belt.beltCTRDecr(right_plain_msg, right_enc_msg, key, 384)'''
